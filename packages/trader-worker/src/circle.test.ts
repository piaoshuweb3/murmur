// Circle Facilitator Service adapter tests — the OFFICIAL hosted x402 facilitator (see circle.ts).
//
// circle.ts is a PURE translation layer between murmur's x402 wire shapes and Circle's
// `POST /v1/facilitator/x402/settle` contract, so every rule Circle enforces can be pinned here with no
// network: the x402Version=2 body (accepted mirrors paymentRequirements, amount == authorization.value),
// the byte-identical bodyStr whose keccak256 becomes the seller proof's bodyHash, the keyless EIP-712
// seller proof (which MUST recover to the payTo key Circle validates against), the exactly-one auth mode,
// and the response normalisation (a 200 carries an OUTCOME — success/pending/failed — while 4xx/5xx are
// rejections; pending is NOT success, so the caller never fulfils without a mined receipt).
...
test("Circle endpoints + protocol version match the documented contract", () => {
  ...
  assert.equal(CIRCLE_X402_VERSION, 2, "Circle requires x402Version 2 (murmur speaks 1 internally)");
});
...
test("builds an x402 v2 body where accepted mirrors paymentRequirements and amount == value", () => {
  ...
  assert.equal(body.paymentRequirements.asset, ARC_USDC);
...
test("includes the paid resource only when a URL is supplied (internal deals omit it)", () => {
  const withRes = buildCircleSettleBody(
    settleArgs({ resourceUrl: "https://api.selfhost.example/signal/pulse", resourceDescription: "arc pulse", resourceMime: "application/json" }),
  ).body;
  assert.deepEqual(withRes.paymentPayload.resource, {
    url: "https://api.selfhost.example/signal/pulse", description: "arc pulse", mimeType: "application/json",
  });
