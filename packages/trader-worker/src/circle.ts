// Circle Facilitator Service adapter — the OFFICIAL hosted x402 facilitator (developers.circle.com/facilitator-service).
//
// WHY THIS EXISTS. Arc had no hosted x402 facilitator, so murmur self-built an OnChainFacilitator that
// holds a dedicated gas wallet and broadcasts every EIP-3009 `transferWithAuthorization` itself (paying
// gas in native USDC). Circle now hosts exactly that role: the buyer still signs the SAME EIP-3009
// authorization, but Circle's relayer screens both parties, submits the transfer, and pays the settlement
// gas — "no relayer keys, no separate gas wallet". This module is the thin, PURE translation layer between
// murmur's x402 wire shapes and Circle's `POST /v1/facilitator/x402/settle` contract, so it is fully unit-
// testable with no network. The network call itself lives in x402.ts (OnChainFacilitator.settleViaCircle).
//
...
//   · Version   : the body's x402Version MUST be 2 (enum [2]); murmur speaks v1 internally, we translate.
//   · Network   : CAIP-2, e.g. "eip155:5042" (Arc mainnet) / "eip155:5042002" (Arc testnet) — NOT murmur's
//                 "arc"/"arc-testnet" tag. The EIP-3009 signature is over the numeric chainId, so translating
//                 the network STRING never invalidates a buyer signature.
...
/** Production base URL. Circle's /settle routes by the body's CAIP-2 network, so one host serves testnet+mainnet. */
...
/** x402 protocol version Circle Facilitator Service requires (murmur speaks 1 internally; we translate). */
...
/** CAIP-2 network id Circle expects for an EVM chain (`eip155:<chainId>`). */
 ...
 * Workers have no Node `Buffer`, so encode via TextEncoder→btoa (same trick as x402.ts b64json) then
 * swap the two URL-unsafe chars and drop padding.
 */
...
/** Circle's `exact`-scheme payment requirements (note: `amount`, not murmur's `maxAmountRequired`). */
...
export interface BuildCircleSettleArgs {
  networkCaip2: string; // eip155:5042 | eip155:5042002
  ...
  /** Optional paid-resource metadata (Arc Pulse sets a real URL; internal deals may omit). */
  resourceUrl?: string;
  resourceDescription?: string;
  resourceMime?: string;
  /**
   * Optional seller-scoped idempotency id (16–128 chars of [A-Za-z0-9_-]). murmur passes the 64-hex
   * authorization nonce, so a retry of the SAME authorization converges on the SAME Circle payment
...
export interface SignSellerProofArgs {
  ...
  chainId: number; // numeric EIP-155 id of the network (5042 / 5042002) — the proof domain's chainId
