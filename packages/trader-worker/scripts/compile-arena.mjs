// Compile contracts/PredictionArena.sol with the npm `solc` (JS) compiler and emit a single artifact
// { abi, bytecode } that the deploy script and the worker's viem integration consume.
// Run: node scripts/compile-arena.mjs
...
const solPath = path.join(root, "contracts", "PredictionArena.sol");
const outDir = path.join(root, "contracts", "build");
const outPath = path.join(outDir, "PredictionArena.json");

const source = fs.readFileSync(solPath, "utf8");
const input = {
  language: "Solidity",
  sources: { "PredictionArena.sol": { content: source } },
  settings: {
...
const unit = out.contracts["PredictionArena.sol"]["PredictionArena"];
