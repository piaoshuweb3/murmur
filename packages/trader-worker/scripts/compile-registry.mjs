// Compile contracts/NeuralReceiptRegistry.sol with the npm `solc` (JS) compiler and emit a single
// artifact { abi, bytecode } that the deploy script and the worker's viem integration consume.
// Run: node scripts/compile-registry.mjs
...
const solPath = path.join(root, "contracts", "NeuralReceiptRegistry.sol");
const outDir = path.join(root, "contracts", "build");
const outPath = path.join(outDir, "NeuralReceiptRegistry.json");

const source = fs.readFileSync(solPath, "utf8");
const input = {
  language: "Solidity",
  sources: { "NeuralReceiptRegistry.sol": { content: source } },
  settings: {
...
const unit = out.contracts["NeuralReceiptRegistry.sol"]["NeuralReceiptRegistry"];
