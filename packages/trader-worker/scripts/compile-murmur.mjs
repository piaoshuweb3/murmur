// Compile contracts/MurmurToken.sol with the npm `solc` (JS) compiler and emit a single artifact
// { abi, bytecode } — same shape and pipeline as compile-war.mjs.
// Run: node scripts/compile-murmur.mjs
...
const solPath = path.join(root, "contracts", "MurmurToken.sol");
const outDir = path.join(root, "contracts", "build");
const outPath = path.join(outDir, "MurmurToken.json");

const source = fs.readFileSync(solPath, "utf8");
const input = {
  language: "Solidity",
  sources: { "MurmurToken.sol": { content: source } },
  settings: {
...
const unit = out.contracts["MurmurToken.sol"]["MurmurToken"];
