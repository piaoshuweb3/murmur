// Compile contracts/MockUSDC.sol (rehearsal/testnet USDC stand-in with EIP-3009) via npm solc.
// Run: node scripts/compile-mock.mjs
...
const solPath = path.join(root, "contracts", "MockUSDC.sol");
const outDir = path.join(root, "contracts", "build");
const outPath = path.join(outDir, "MockUSDC.json");
...
const unit = out.contracts["MockUSDC.sol"]["MockUSDC"];
